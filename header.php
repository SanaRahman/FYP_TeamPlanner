...
<link rel="shortcut icon"   type="image/png" href="assets/images/logo2.png">
  ...
  if(isset($_GET['page'])){
  $t=explode('/',$_GET['page']);
  if(count($t)>1){  $string=$t[1];}
  else $string=$_GET['page'];
}
else{$string='login';}

    $title = isset($_GET['page']) ? ucwords(str_replace("_", ' ', $string)) : "Home";
  ?>
  <title><?php echo $string ?> | <?php echo  "Team Planner"//$_SESSION['system']['name'] ?></title>
